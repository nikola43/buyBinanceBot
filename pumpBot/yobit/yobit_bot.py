...
if __name__ == "__main__":
    ...
    while True:
        ...
        if coin is not None:
            print(coin)
            selected_symbol = coin.lower() + "_btc"
            webbrowser.open("https://yobit.net/en/trade/"+coin.upper()+"/BTC", new=2)
            break
    ...
    buy_price = round(initial_price + (initial_price * 0.2) / 100, 8)
    buy_quantity = float(round(float(btc_balance) / buy_price, 8))
    buy_quantity_with_fee = float(round(buy_quantity - (buy_quantity * 10) / 100, 8))
    ...
    sell_quantity = float(float(round(buy_quantity_with_fee - (buy_quantity_with_fee * 10) / 100, 8)))
    ...
    while True:
        ...
        if "return" in r and symbol in r["return"]["funds"]:
            selectedSymbolBalance = float(r["return"]["funds"][selected_symbol])
        # get symbol info
        # GET PRICE
        symbol_info = json.loads(requests.get("https://yobit.net/api/3/ticker/" + selected_symbol).text)
        current_price = round(float(symbol_info[selected_symbol]["last"]), 8)
        ...
        if current_price > initial_buy_price + ((initial_buy_price * max_profit_percent) / 100):
            take_profit_price = float(round(float(current_price) - (float(current_price) * 0.2), 8))
            #cancel_order(order)
            params = {
                "pair": selected_symbol,
                "type": "sell",
        ...
        if current_price > initial_buy_price and current_price < (high_price - ((high_price * 25) / 100)):
            take_profit_price = float(round(float(current_price) - (float(current_price) * 0.2), 8))
        ...
        print_current_status(current_price, last_price, high_price, stop_loss_price,initial_buy_price, target_sell_price)
